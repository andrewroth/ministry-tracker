module MonthlyReportsHelper
  ...
  def get_stat(stat_to_collect)
    ...
    when AUTO_COLLECT_NUM_MIN_DISCS
      ...
      ministry_disciples_ids = MinistryInvolvement.find(:all, :conditions => { :end_date => nil, :ministry_id => campus_ministry_ids, :ministry_role_id => ministry_disciples_role_ids }, :group => :person_id).collect(&:person_id)
      campus_disciples = CampusInvolvement.find(:all, :conditions => {:campus_id => campus_id, :person_id => ministry_disciples_ids}, :include => :person).collect(&:person)
