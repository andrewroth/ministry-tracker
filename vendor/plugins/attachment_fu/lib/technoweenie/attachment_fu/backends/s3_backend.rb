      ...
      module S3Backend
        ...
        def self.included(base) #:nodoc:
          ...
          begin
            ...
            config = YAML.load(ERB.new(File.read(@@s3_config_path)).result)[RAILS_ENV] || YAML.load(ERB.new(File.read(@@s3_config_path)).result)['production']
            @@s3_config = config.symbolize_keys
          rescue
           raise ConfigFileNotFoundError.new("File %s not found for #{RAILS_ENV} environment" % @@s3_config_path)
          end
