      ...
      module ClassMethods
        ...
        def rebuild!
          ...
          set_left_and_rights = lambda do |node|
            ...
            # find
            find(:all, :conditions => ["#{quoted_parent_column_name} = ? #{scope.call(node)}", node], :order => "#{quoted_left_column_name}, #{quoted_right_column_name}, #{_(:id)}").each{|n| set_left_and_rights.call(n) }
            # set right
          ...
          # Find root node(s)
          root_nodes = find(:all, :conditions => "#{quoted_parent_column_name} IS NULL", :order => "#{quoted_left_column_name}, #{quoted_right_column_name}, #{_(:id)}").each do |root_node|
