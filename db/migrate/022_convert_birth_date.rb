class ConvertBirthDate < ActiveRecord::Migration
  def self.up
    Person.find(:all).each do |p|
      p.birth_date = Date.parse(p.birth_date).to_s unless p.birth_date.nil? || p.birth_date.is_a?(Date)
      p.graduation_date = Date.parse(p.graduation_date).to_s unless p.graduation_date.nil? || p.graduation_date.is_a?(Date)
      p.save
    end
