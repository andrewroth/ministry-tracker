class AddLabelMultiplePermissions < ActiveRecord::Migration
  NEW_PERMISSIONS = [{ :description => "Set multiple labels", :controller => "people", :action => "set_label_multiple" },
                     { :description => "Remove multiple labels", :controller => "people", :action => "remove_label_multiple" }
                     ]
