class GroupInvitationsController < ApplicationController
  ...
  def get_and_check_invitation
    ...
    if @invitation.blank? || @invitation.has_response? || @invitation.group.blank?
      if @invitation.present? && @invitation.has_response?
        flash[:notice] = "<strong>This invitation has already been responded to.</strong><br/><br/>If this is a mistake we'd still love you to join, so go ahead and click JOIN A GROUP below to find your group and join!"
      elsif @invitation.group.blank?
        flash[:notice] = "<strong>We're sorry, the group you're trying to join doesn't exist anymore.</strong><br/><br/>We'd still love you to join a group though, so go ahead and click JOIN A GROUP below to find your group and join!"
      else
        flash[:notice] = "<strong>We're sorry, something went wrong with your group invitation.</strong><br/><br/>We'd still love you to join though, so go ahead and click JOIN A GROUP below to find your group and join!"
      end
      access_denied(true)
      return
    end
