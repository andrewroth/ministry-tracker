...
class UserMailer < ActionMailer::Base
  ...
  def confirm_email(email)
    ...
    @subject      = "#{t("misc.email_prefix")} #{t("emails.confirm_email_subject")}"
    @sent_on      = Time.now
  ...
  def signup_finished_email(email, link, joined_collection_group)
    @recipients   = "#{email}"
    @from         = Cmt::CONFIG[:email_from_address]
    @subject      = "#{t("misc.email_prefix")} #{t("emails.signup_finished_subject")}"
    @sent_on      = Time.now
  ...
  def signup_confirm_email(email, link)
    @recipients   = "#{email}"
    @from         = Cmt::CONFIG[:email_from_address]
    @subject      = "#{t("misc.email_prefix")} #{t("emails.signup_confirm_subject")}"
    @sent_on      = Time.now
    @content_type = "text/html"
    @body[:link]  = link
  end
  ...
  def group_join_email(requested, interested, group_name, leader_first_name, leader_email, 
    ...
    @from        = Cmt::CONFIG[:email_from_address]
    if requested
      @subject     = "#{t("misc.email_prefix")} #{t("emails.group_join_requested_subject", :group_name => group_name)}"
    elsif interested
      @subject     = "#{t("misc.email_prefix")} #{t("emails.group_join_interested_subject", :group_name => group_name)}"
    else
      @subject     = "#{t("misc.email_prefix")} #{t("emails.group_join_email", :group_name => group_name)}"
    end
    @sent_on     = Time.now
  ...
  def summer_report_submitted(review, base_url)
    ...
    @from = Cmt::CONFIG[:email_from_address]
    @subject = "#{t("misc.email_prefix")} #{review.summer_report.person.full_name} submitted summer schedule"
    @sent_on = Time.now
  ...
  def summer_report_reviewed(summer_report, base_url)
    ...
    if summer_report.approved?
      @subject = "#{t("misc.email_prefix")} Your summer schedule was approved!"
    elsif summer_report.disapproved?
      @subject = "#{t("misc.email_prefix")} Your summer schedule was disapproved..."
    end
  ...
  def group_invitation(group_invitation, base_url)
    ...
    @subject = "#{t("misc.email_prefix")} #{t("emails.group_invitation_subject", :sender_name => group_invitation.sender_person.first_name, :group_name => group_invitation.group.name
  ...
  def group_invitation_decline(group_invitation, base_url)
    ...
    @subject = "#{t("misc.email_prefix")} #{t("emails.group_invitation_decline_subject", :subject_name => group_invitation.recipient_email, :group_name => group_invitation.group.name)}"
  ...
  def created(person, ministry, added_by, password)
    ...
    @from         = Cmt::CONFIG[:email_from_address]
    @subject     = "#{t("misc.email_prefix")} #{t("emails.created_student_subject")}"
    @sent_on     = Time.now
    @body[:person] = person
