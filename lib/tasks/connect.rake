namespace :connect do
  task :import_contacts => :environment do
    ...
    begin
      connect = CiviCRM::RestAPI.new(:log_tags => ['IMPORT CONTACTS'])

      CiviCRM.log :info, "Starting contact import..."
      ...
      unimported_contacts = connect.get(:Contact,
                                        :with => { dbm[:imported_to_pulse][:field] => 0, :row_count => 1 },
      ...
      CiviCRM.log :info, "#{unimported_contacts.size} contacts to import."

      if unimported_contacts.size == 0
        CiviCRM.log :info, "Exiting."
        next
      end
    ...
    unimported_contacts.each_with_index do |contact, i|
      begin
        CiviCRM.log :info, "Importing contact #{i+1} of #{unimported_contacts.size} with id #{contact.id}..."
        if SurveyContact.all(:conditions => { :email => contact.email }).present?
          CiviCRM.log :warn, "Contact #{contact.id} with email '#{contact.email}' already exists in the Pulse, skipping the import of this contact!"
          next
        end
        ...
        survey_contact = SurveyContact.new({
          ...
          :gender_id => contact.gender_id.present? ? contact.gender_id : 0,
          :email => contact.email && contact.email.include?('no_value_set') ? '' : contact.email,
          :cellphone => contact.phone.length < 7 ? '' : contact.phone,
          :campus_id => school_contact.external_identifier,

          # custom values info
          # :year => year_options[contact.custom_values[dbm[:year][:id]].attr('0')].try(:label),
        ...
        CiviCRM.log :error, "Failed to import contact #{contact.try(:id)}", e
        ...
        begin
          ...
          CiviCRM.log :error, "Failed to update contact #{contact.try(:id)} as imported", e
        end
      end
    end

    CiviCRM.log :info, "Finished import of #{unimported_contacts.size} contacts, exiting."
  end
end