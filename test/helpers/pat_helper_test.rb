...
#    Factory(:user_11)
  ...
  def test_project_acceptance_totals
    ...
    assert_equal 1, results_by_campus["UCD"][:total].to_i
    assert_equal 2, results_by_campus["Wy"]["project 2"].to_i
    assert_equal 1, results_by_project["project 1"]["UCD"].to_i
    assert_equal 2, results_by_project["project 1"][:total].to_i
    assert_equal 2, results_by_project["project 2"]["Wy"].to_i
    assert_equal 2, results_by_project["project 2"][:total].to_i
  end
  ...
  def test_project_applying_totals
    ...
    assert_equal 1, results_by_project["project 1"]["UCD"].to_i
    assert_equal 2, results_by_project["project 1"][:total].to_i
    assert_equal 1, results_by_project["project 2"]["Wy"].to_i
    assert_equal 1, results_by_project["project 2"][:total].to_i
  end
end
